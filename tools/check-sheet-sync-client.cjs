...
const run=extract('syncMissingSheetOrdersToSupabase');
const refresh=html.match(/document.getElementById\('autoPlanRefreshBtn'\).addEventListener\('click', async \(\)=>\{([\s\S]*?)\n\}\);/)[1];
...
const ctx=vm.createContext({SUPABASE_VARIANT:true,PUBLIC_VIEW_MODE:false,sheetPlanSyncInFlight:false,
 ...
 console:{warn:()=>{}},fetchBaoHanh:async()=>{},renderAutoPlan:()=>{},
 fetchAutoPlan:async()=>{throw Error('legacy Sheet mutation must not run for Supabase')},
 fetchOrderGroups:async()=>{throw Error('legacy group refresh unexpected')},
 refreshGuestDashboard:async()=>{calls.push('guest')}
});
vm.runInContext(run+'; globalThis.sync=syncMissingSheetOrdersToSupabase;globalThis.refresh=async()=>{'+refresh+'};',ctx);
(async()=>{
 ...
 calls=[];await ctx.refresh();assert.equal(calls.length,1);assert.equal(calls[0].name,'khsx_reconcile_sheet_plan');
 allowed=false;calls=[];await ctx.refresh();assert.equal(calls.length,0);
 ctx.PUBLIC_VIEW_MODE=true;await ctx.refresh();assert.deepEqual(calls,['guest']);
 console.log('PASS manager refresh uses guarded sync; other roles read; guest stays on guest loader');
