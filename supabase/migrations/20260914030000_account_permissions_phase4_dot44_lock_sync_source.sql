-- Phan quyen tai khoan - Dot 4.4 (Co che 1: gan permission_key vao 2 RPC da
-- co san cho, khong tao RPC moi). Khong doi hanh vi cho chu tai khoan (luon
-- bypass qua private.khsx_is_permission_owner). Tai khoan chua duoc tick
-- quyen tuong ung se bi chan dung nhu truoc (Phuoc hien chua co
-- khsx_lock_plan/khsx_unlock_plan/khsx_sync_source nen hanh vi voi Phuoc
-- khong doi cho toi khi duoc tick).

-- 1) khsx_set_day_locks: nhanh 'plan' truoc day chan cung theo role
-- (chi quan_ly_2 bi tu choi, quan_ly nao cung lam duoc). Doi sang dung
-- permission_key khsx_lock_plan/khsx_unlock_plan, giong het cach nhanh
-- 'progress' da lam voi progress_lock/progress_unlock.
...
-- 2) khsx_reconcile_sheet_plan_impl: truoc day chan cung role='quan_ly' (chi
-- full manager, khong phan biet duoc ai). Doi sang permission_key
-- khsx_sync_source (owner van bypass nhu cu qua khsx_has_permission).
create or replace function private.khsx_reconcile_sheet_plan_impl(p_source_orders jsonb, p_source_read_at timestamp with time zone, p_pending_order_ids text[] default '{}'::text[])
returns jsonb
...
declare
  ...
  v_count integer;
begin
  if v_actor is null or not private.khsx_has_permission('khsx_sync_source', v_actor) then
    raise exception using errcode='42501',message='KHSX_SYNC_SOURCE_FORBIDDEN';
  end if;
...
-- 3) Trigger khsx_guard_ql2_day_lock: chan CUNG theo role quan_ly_2, doc lap
-- voi he thong quyen moi - phat hien khi test Co che 1 (khsx_lock_plan da
-- tick van bi chan boi trigger nay, khong lien quan RPC). Doi sang dung dung
-- permission_key giong RPC khsx_set_day_locks. Nhan tien va luon lo hong cu:
-- progress_unlock (Dot 4.3) cung bi trigger nay chan cung cho quan_ly_2, chua
-- ai phat hien vi chua ai thu mo khoa tien do voi tai khoan quan_ly_2.
...
begin
  ...
  if v_role='quan_ly_2' then
    if tg_op='INSERT' and coalesce(new.plan_locked,false) and not private.khsx_has_permission('khsx_lock_plan') then
      raise exception using errcode='42501',message='LOCKED_NO_UNLOCK';
    end if;
    if tg_op='UPDATE' and old.plan_locked is distinct from new.plan_locked then
      if new.plan_locked and not private.khsx_has_permission('khsx_lock_plan') then
        raise exception using errcode='42501',message='LOCKED_NO_UNLOCK';
      elsif not new.plan_locked and not private.khsx_has_permission('khsx_unlock_plan') then
        raise exception using errcode='42501',message='LOCKED_NO_UNLOCK';
      end if;
    end if;
    if tg_op='UPDATE' and coalesce(old.progress_locked,false) and not coalesce(new.progress_locked,false)
       and not private.khsx_has_permission('progress_unlock') then
      raise exception using errcode='42501',message='LOCKED_NO_UNLOCK';
    end if;
    if tg_op='DELETE' and (coalesce(old.plan_locked,false) or coalesce(old.progress_locked,false)) then
      raise exception using errcode='42501',message='LOCKED_NO_UNLOCK';
    end if;
  end if;
