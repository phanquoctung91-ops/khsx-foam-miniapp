-- Giai đoạn đầu chưa đủ tài khoản nhân sự trong catalogue khsx_workers cho May/Đóng gói.
-- Trước đây: Quản lý chọn người hoàn thành nhưng người đó không có (hoặc không active)
-- trong khsx_workers -> RPC từ chối hẳn (WORKER_REQUIRED), số liệu KHÔNG được lưu.
-- Sau: nếu người thao tác là Quản lý, không chặn nữa -- quy log + KPI (nếu có worker_id
-- trong hồ sơ Quản lý) về chính Quản lý thay vì huỷ toàn bộ giao dịch. Nhân viên thường
-- vẫn bị chặn như cũ (không đổi hành vi cho vai nhân viên).
  ...
  if p_stage in ('may','dong_goi') then
    ...
    if v_worker is not null and not exists(select 1 from public.khsx_workers w where w.id=v_worker and w.stage=p_stage and w.active) then
      if v_is_manager then
        -- Quản lý xác nhận nhưng người chọn chưa có trong catalogue (giai đoạn đầu
        -- chưa đủ tài khoản để test) -> vẫn ghi, quy log/KPI về chính Quản lý.
        v_worker := v_profile_worker;
      else
