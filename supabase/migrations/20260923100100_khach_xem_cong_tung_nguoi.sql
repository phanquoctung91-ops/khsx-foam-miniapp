-- Sửa 2026-09-23 (anh Tùng duyệt "cho khách xem hết"): gói dữ liệu khách thêm bảng công
-- (khsx_stage_credits) và danh mục thợ (khsx_workers), để tab Năng lực của khách hiện được công
-- May / Đóng gói theo từng người. Trước đây khách không có bảng công nên thẻ hiện "Đang tải…"
-- mãi không ra số.
--
-- Hệ quả anh Tùng đã chấp nhận: ai có link khách đều xem được công từng người.
-- Phần còn lại của hàm giữ nguyên từng chữ so với bản đang chạy.

CREATE OR REPLACE FUNCTION "public"."khsx_guest_dashboard_v116"() RETURNS "jsonb"
...
select jsonb_build_object(
  ...
  -- Anh Tùng duyệt 23/09/2026: khách xem được cả công từng người ở tab Năng lực.
  'stage_credits',coalesce((
    select jsonb_agg(to_jsonb(q)) from (
      select c.order_id,c.work_date,c.stage,c.worker_id,c.quantity,c.source,c.kpi_team
      from public.khsx_stage_credits c
      join public.khsx_orders o on o.id=c.order_id and o.deleted_at is null
      order by c.work_date,c.order_id,c.stage
    ) q
  ),'[]'::jsonb),
  'workers',coalesce((
    select jsonb_agg(to_jsonb(q)) from (
      select w.id,w.display_name,w.stage,w.active
      from public.khsx_workers w order by w.id
    ) q
  ),'[]'::jsonb)
);
$$;
