-- Phan quyen tai khoan - Dot 4.5: gop khsx_sync_source + settings_import_khsx_file
-- thanh 1 cong (da chot voi anh Tung) - RPC dung chung 1 ham cho "dong bo Sheet"
-- va "nap file KHSX tay", nen chi can 1 trong 2 permission_key la du, khong bat
-- phai co ca 2. Khong doi hanh vi cho chu tai khoan (van bypass qua
-- khsx_has_permission).
...
begin
  if v_actor is null or not (
    private.khsx_has_permission('khsx_sync_source', v_actor)
    or private.khsx_has_permission('settings_import_khsx_file', v_actor)
  ) then
